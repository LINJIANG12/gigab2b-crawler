# GigaB2B (大健云仓 / GigaCloud) 全站商品数据采集与逆向分析爬虫

> **English Title:** GigaCloud B2B (GigaB2B) E-Commerce Full-Site Product Scraper & Reverse Engineering Crawler  
> **关键词 / Keywords:** `GigaB2B` `大健云仓` `GigaCloud` `跨境电商爬虫` `外贸供应链` `一件代发` `海外仓库存` `API逆向` `价格监控` `Python爬虫` `SPU/SKU采集`

基于 Python 3.14 + 官方后端 API 逆向驱动的跨境大件电商供应链平台（**GigaB2B / 大健云仓 / GigaCloud Technology**）工业级商品数据采集、全量海外仓库存监控与自动化导出系统。

> 💬 **写在前面**：做这个项目是因为缺钱接单，但是东西做完了买家又不要了。遂开源。

---

## 🌟 核心特性

- **官方 API 驱动**：直接调用官方 JSON 接口 (`product/list/search`, `product/info/info/baseInfos`, `product/info/price/list`)，速度达 **12~18 件/秒**。
- **37 个全量独立字段**：
  - **基础信息**：商品 ID、货号/SKU、商品标题、分类全路径、店铺名称、店铺代码、商品链接
  - **价格与库存**：B2B批发价、折扣价、MSRP、起订量、总现货库存、各海外分仓库存分布
  - **物流与履约**：一件代发预估运费、云仓运费区间、出库处理时效、运输派送时效、是否 LTL 大件物流、总磅重、总体积
  - **规格与属性**：主颜色、主材质、原产国、UPC 编码、产品组装尺寸、外包装箱规长宽高与毛重
  - **图文与资料**：质量评级与退货率、采购订单量、售后质保政策、说明书手册下载直链、核心卖点提要、详细描述
  - **媒体与多变体**：高清主图、全量副图（多达 15~25 张）、多变体明细（子 SKU 价格与库存）
  - **状态与诊断**：在售与权限状态、数据状态与原因说明（如全仓缺货、需申请供应商授权）
- **持久化与断点续爬**：内置 SQLite WAL 模式数据库 (`gigab2b.db`)，支持任意中断后增量继续爬取。
- **大规模分卷 Excel 与 CSV 导出**：自动分卷（每 50,000 条切卷）、UTF-8 BOM CSV、文件占用自动避让与样式自适应美化。
- **全站 217 个分类全覆盖（SPU vs SKU 精确数据说明）**：
  - **独立商品主体 (SPU)**：全站物理去重后实际为 **23,232 款** 独立商品；
  - **可售规格变体 (SKU)**：按颜色、尺寸展开后全量为 **8.3 万 ~ 9.6 万条** 独立规格明细；
  - **前台分类累加 (9.68 万)**：官方前台 217 个分类展示计数累加为 **96,818 条**（原因：① 包含了颜色/尺寸等多变体展开；② 平台运营策略下一件商品平均跨 4.5 个分类重复挂载曝光）；
  - 系统支持 **SPU 汇总导出**（`--export-only`）与 **SKU 变体展开导出**（`--export-skus`），满足不同铺货与分析需求。
- **渠道保护/需申请授权商品穿透探测**：创新采用**搜索筛选二分价格预言机（Binary Search Oracle）**，彻底突破供应商渠道保护与价格隐藏限制，实现分精度底价间接反推与 100% 价格覆盖。

---

## 💡 核心技术：渠道保护商品价格获取机制

在 GigaB2B 平台中，约 **55% 的商品开启了供应商渠道保护**（需单独申请合作授权）。对于此类商品，官方详情接口会将价格隐藏并返回 `price_visible: false`。

本项目针对该难点实现了专有的逆向突破机制：

1. **服务端筛选特性发现**：
   - 官方搜索接口 (`product/list/search`) 中的价格区间筛选器 (`price_min` / `price_max`) 在服务端对渠道保护商品**完全生效**。
2. **二分价格预言机（Binary Search Oracle）**：
   - 锁定商品指定 SKU，在 `[$0.01, $3000.00]` 价格空间内发起 18 轮二分切片查询。
   - 仅通过区间命中反馈（是否出现在结果中），即可在 **8 秒内**将价格收敛至 **$0.01（分精度）** 真实底价。
3. **数据补齐与来源标识**：
   - 探测结果自动回填至数据库 `price` 字段，并在 `status_reason` 中清晰标记为 `渠道保护价(搜索筛选二分探测)`，确保数据 100% 完整可用且来源可溯。

```bash
# 运行渠道保护商品价格二分探测与补齐
py -3.14 tools/crawl_protected_prices.py
```

---

## 🚀 快速开始

### 1. 安装依赖
```bash
pip install -r requirements.txt
```

### 2. 配置登录凭据 (Cookie)
1. 在 Edge / Chrome 浏览器中打开 [https://www.gigab2b.com](https://www.gigab2b.com) 并登录。
2. 按 `F12` 打开开发者工具 -> 切换到 **Console (控制台)**。
3. 输入 `copy(document.cookie)` 并按回车。
4. 将复制的内容粘贴保存到项目根目录的 `cookie.txt` 文件中即可。

### 3. 一键运行采集

- **运行 100 条示例商品测试**：
  - 双击 `爬取100个示例.bat`，或在终端运行：
    ```bash
    py -3.14 crawl_100_samples.py
    ```
- **启动全站全量数据采集**：
  - 双击 `启动采集.bat`，或在终端运行：
    ```bash
    py -3.14 main.py --workers 20
    ```
- **查看当前数据库状态与采集进度**：
  ```bash
  py -3.14 main.py --status
  ```
- **从已有数据库重新导出 Excel/CSV 报表**：
  ```bash
  py -3.14 main.py --export-only
  ```
- **测算全站分类与商品数量分布**：
  ```bash
  py -3.14 calc_category_totals.py
  ```

---

## 📁 项目结构

```text
├── docs/                     # 逆向分析、风控绕过与技术文档
├── tools/                    # 逆向探测、数据审计、价格修复与诊断分析工具集
├── data/                     # 采集结果输出目录（Excel / CSV）
├── images/                   # 商品高清图片下载目录
├── config.py                 # 全局配置（并发数、超时、重试与路径）
├── cookie_manager.py         # Cookie 自动管理、登录态校验与 Edge 本地提取
├── database.py               # SQLite 持久化、任务队列与断点续爬管理
├── parser.py                 # 37 字段深度结构化解析器
├── crawler.py                # 全站多线程并发采集调度引擎
├── exporter.py               # Excel / CSV 结构化分卷导出模块
├── main.py                   # CLI 命令行主入口
├── crawl_100_samples.py      # 快速 100 条示例验证脚本
├── calc_category_totals.py   # 全站分类商品体量测算工具
├── cookie.txt.example        # Cookie 格式模板
├── requirements.txt          # Python 依赖清单
├── 启动采集.bat              # Windows 一键全量采集脚本
├── 爬取100个示例.bat         # Windows 一键示例采集脚本
├── 提取Edge登录凭据.bat      # Windows 一键提取 Cookie 脚本
└── LICENSE                   # MIT 开源许可证
```

---

## 📄 开源许可证 (License)

本项目采用 [MIT License](LICENSE) 授权，允许任何人免费、自由地用于学习、二次开发与商业用途。

---

## ⚠️ 免责声明 (Disclaimer)

1. **仅供学习研究**：本项目开源的代码仅供 Python 编程学习、Web API 逆向与爬虫架构技术研究交流使用，严禁将本项目用于任何非法盈利、商业竞争、破坏服务器或恶意攻击等违法违规行为。
2. **合规与责任自负**：使用者在使用本项目代码时，应自行评估并严格遵守相关法律法规以及目标网站的《服务条款》与《Robots 协议》。请务必合理控制请求频率，禁止高频施压造成目标服务器负载过大。因使用者滥用本项目代码所引发的任何直接或间接法律纠纷、商业损失、账号封禁等后果，均由使用者自行承担，项目作者不承担任何连带责任。
3. **数据产权归属**：本项目代码中涉及的商品数据、品牌名称、商标、图片及相关资料，其版权与知识产权均归原平台（GigaCloud / GigaB2B）及其合法商户所有。本项目仓库本身不包含、不托管、亦不出售任何受版权保护的商业数据。
4. **侵权与下架说明**：若相关权利方认为本项目涉及侵害其合法权益，请通过 GitHub Issue 提交通知，项目作者将在核实后第一时间积极配合修改或下架相关内容。
