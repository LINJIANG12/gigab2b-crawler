...
res = s.post(url, json={"page": 1, "limit": 30, "search_dimension": 1, "scene": 1}).json()
cat_tree = res.get('data', {}).get('category', [])

print("Sample category item structure:")
if cat_tree:
    print(json.dumps(cat_tree[0], ensure_ascii=False, indent=2))
